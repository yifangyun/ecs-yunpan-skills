...
```bash
DISK_360_CLI='npx -y -p @aicloud360/mcp-server-disk@<exact-version> 360disk'
```

如果用户明确要求全局安装，才使用 `npm install -g @aicloud360/mcp-server-disk`。
...
```bash
# cmd: 从本地文件读取内容并保存
type report.md | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file save --stdin --dest /文件夹/ --filename report.md

# PowerShell: 从本地文件读取内容并保存
Get-Content report.md -Raw | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file save --stdin --dest /文件夹/ --filename report.md

# cmd: 将本地 JSON 整文件写入云盘配置（file config；勿用 cat，cmd 无该命令）
type local-config.json | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file config --path /mcp/app.json --command config:write --type json --stdin

# PowerShell: 同上
Get-Content .\local-config.json -Raw | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file config --path /mcp/app.json --command config:write --type json --stdin

# cmd: 追加本地文件内容到云盘（勿用 cat）
type local_log.md | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file append /项目/开发日志.md --stdin

# PowerShell: 同上
Get-Content .\local_log.md -Raw | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file append /项目/开发日志.md --stdin

# cmd：file exists --stdin 勿用 echo '...' |（单引号会进管道，JSON 非法）；先把 UTF-8 JSON 写入 files.json
type files.json | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file exists --path /目录/ --stdin

# PowerShell：同上
Get-Content .\files.json -Raw -Encoding utf8 | npx -y -p @aicloud360/mcp-server-disk@latest 360disk file exists --path /目录/ --stdin
```
