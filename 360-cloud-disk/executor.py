...
class MCPExecutor:
    ...
    def __init__(self):
        # 连接模式：local/npx/http
        self.mode = os.getenv("MCP_MODE", "local").lower()
        ...
        if self.mode == "local":
            ...
            # HTTP 模式：使用 HTTP API
            self.http_url = os.getenv("MCP_HTTP_URL", "")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  MCP_MODE=http MCP_HTTP_URL=https://api.example.com/mcp python executor.py user-info")
        sys.exit(1)
